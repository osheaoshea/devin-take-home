...
- On sign-in: read `groups` claim → resolve app roles via a **group→role map** (config: `ENTRA_GROUP_MAP` env or small table) → persist a snapshot of resolved roles on the `users` row (so audit entries record the roles the actor held *at the time*).
...
Enforcement at **two layers**: route/action middleware (`requireRole`, `requirePermission`) *and* query scoping in `lib/db` accessors. Hidden buttons are not access control.
...
- **Identity:** mock IdP issues fake group claims so the *real* group→role mapping path runs in demo.
